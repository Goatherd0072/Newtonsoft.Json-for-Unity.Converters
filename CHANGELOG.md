...
- 为Newtonsoft Json的默认设置配置文件添加更多选项，更改文件生成按钮的菜单栏目录。 
- 重组仓库以遵循 Unity Package Manager (UPM) 标准目录结构。
  ([#1](https://github.com/Goatherd0072/Newtonsoft.Json-for-Unity.Converters/pull/1))
- 将包内容移动到仓库根目录，以便于通过 Git URL 安装。
- 更新了包元数据（名称、版本、作者）。
- 更新了 README，增加了通过 Git URL 安装的说明。
- 调整部分文件命名。

EN:

- Added more options to the default configuration for Newtonsoft Json, and changed the menu location for the file generation button.
- Reorganized repository to follow Unity Package Manager (UPM) standard directory
  structure. ([#1](https://github.com/Goatherd0072/Newtonsoft.Json-for-Unity.Converters/pull/1))
...
- Fixed converter lookups collisions when multiple assemblies converters
  ...
  Thanks [@Erifirin](https://github.com/Erifirin) for the pull
  request ([#90](https://github.com/jilleJr/Newtonsoft.Json-for-Unity.Converters/pull/90))
...
- Fixed UnityConvertersConfigEditor scriptable object asset throwing error after
  ...
  Thanks [@EpsilonD3lta](https://github.com/EpsilonD3lta) for the
  implementation ([#83](https://github.com/jilleJr/Newtonsoft.Json-for-Unity.Converters/pull/83))

## 1.6.0 (2023-12-03)

- Added converters
  for [Unity.Mathematics](https://docs.unity3d.com/Packages/com.unity.mathematics@1.3/manual/index.html).
...
- Fixed converter types taking long time to load, sometimes causing lag spikes
  ...
  paths and making use of
  Unity's [TypeCache](https://docs.unity3d.com/2023.3/Documentation/ScriptReference/TypeCache.html).
  ...
  "Edit > Json.NET converters
  settings..." ([#79](https://github.com/jilleJr/Newtonsoft.Json-for-Unity.Converters/pull/79))

    - Thanks [@tomkail](https://github.com/tomkail) for the excellent
      issue ([#77](https://github.com/applejag/Newtonsoft.Json-for-Unity.Converters/issues/77)).
    - Thanks [@Mefodei](https://github.com/Mefodei) for the
      inspiration ([#78](https://github.com/applejag/Newtonsoft.Json-for-Unity.Converters/pull/78))
...
- Added support for `UnityEngine.AddressableAssets.AssetReferenceT<T>`, in
  ...
  Thanks [@kyverr](https://github.com/kyverr) for the
  implementation ([#71](https://github.com/jilleJr/Newtonsoft.Json-for-Unity.Converters/pull/71))
...
- Changed the following modules to be automatically excluded from compilation
  ...
  Thanks [@SolidAlloy](https://github.com/SolidAlloy) for the
  implementation ([#60](https://github.com/jilleJr/Newtonsoft.Json-for-Unity.Converters/pull/60))
...
- Fixed `float` precision error when
  reading/prasing. ([#46](https://github.com/jilleJr/Newtonsoft.Json-for-Unity.Converters/issues/46),
